...
public:
	...
	public:
		...
		void FindText(LPCTSTR szText, BOOL bMC, BOOL bMW, BOOL bMRegEx)
		{
        ...
        BOOL        bMatchWord;             // match words
        BOOL        bMatchCase;             // match case
        BOOL        bMatchRegEx;            // match regular expression
	};
	...
	BOOL	FindTextInFile(LPCTSTR szFile, CFileFinder::CFindOpts &opts);
	BOOL	FindTextInFile2(LPCTSTR szFile, CFileFinder::CFindOpts &opts);
    void    AddFoundText (const char* pTextBuf, int nRow);
    ...
    CStringList& GetStrFound ()   { return m_strFound; }
	...
	void	AddProjectFile (CString strFileProject);
private :
    static CString AddBackSlash(LPCTSTR szFolderPath, BOOL bInverted = FALSE);
    BOOL IsBinary (const unsigned char* p, int nLen);
    BOOL IsWord (const char *pText, char *p, int nLen);
    BOOL IsWordCharForSel(char ch);
	bool IsFileInProject (CString strFileProject);
private:
    CStringList     m_strFound;
	CStringArray	_aFilesFound;
	bool			_bStopSearch;
	FILEFINDERPROC	_pFileFinderProc;
	void			*_pCustomParam;
	CString			_sSearchingFolder;
	CStringList	    m_listFilesInProject;
};
