...
static bool IsEndNextWord (Accessor &styler, int nPos);
static bool IsGlobalPreviousWord (Accessor &styler, int nPos);
...
static void FoldLuaDoc(unsigned int startPos, int length, int /* initStyle */, WordList *[],
	...
	for (unsigned int i = startPos; i < lengthDoc; i++) {
		...
		if (style == SCE_LUA_WORD) {
			if (ch == 'i' || ch == 'd' || ch == 'f' || ch == 'e' || ch == 'r' || ch == 'u' || ch == 's' || ch == 'o') {
				int n = 0; char s[10] = "";
				for (unsigned int j = 0; j < 8; j++, n++) {
				...
				if ((strcmp(s, "object") == 0) || (strcmp(s, "switch") == 0) || (strcmp(s, "if") == 0) || (strcmp(s, "do") == 0) || (strcmp(s, "function") == 0) || (strcmp(s, "repeat") == 0)) {
					if (!strcmp(s, "object") && IsEndNextWord(styler, i + n));
					else
					if (!strcmp(s, "switch") && IsEndNextWord (styler, i + n));
					else
					if (!strcmp(s, "function") && IsGlobalPreviousWord (styler, i-1));
					else					
						levelCurrent++;
				}
...
static bool IsEndNextWord (Accessor &styler, int nPos)
{
	...
	p[0] = '\0';
	while (styler[nPos] != '\0' && styler[nPos] != '\n' && styler[nPos] != '\r')   {
		p[n] = styler[nPos];
		n++; nPos++;
	}
	p[n] = '\0';
...
static bool IsGlobalPreviousWord (Accessor &styler, int nPos)
{
	...
	p[1023] = '\0';
	while (nPos >= 0 && styler[nPos] != '\n' && styler[nPos] != '\r')   {
		p[n] = styler[nPos];
		n--; nPos--;
	}
	if (n != 1022)   {
		if (strstr (&p[n+1], "global"))
			b = true;
	}
	delete p;
	return b;
