...
void CDiagramEditor::DrawTabNumber( CDC* dc, double zoom ) const
{
...
void CDiagramEditor::MakeSameSizeSelectedHorizontal()
/* ============================================================
	Function :		CDiagramEditor::MakeSameSizeSelected
	Description :	Makes all selected objects the same size as 
					the top selected object in the data container.
   ...
   ============================================================*/
{
	...
	if( GetSelectCount() > 1 )
	{
		...
		if( obj )
		{
			...
			while( ( obj = m_objs->GetAt( count++ ) ) )
				if( obj->IsSelected() )
					obj->SetRect( obj->GetLeft(), obj->GetTop(), obj->GetLeft() + width, obj->GetBottom());
...
void CDiagramEditor::MakeSameSizeSelectedVertical()
/* ============================================================
	Function :		CDiagramEditor::MakeSameSizeSelected
	Description :	Makes all selected objects the same size as 
					the top selected object in the data container.
   ...
   ============================================================*/
{
	...
	if( GetSelectCount() > 1 )
	{
		...
		if( obj )
		{
			...
			while( ( obj = m_objs->GetAt( count++ ) ) )
				if( obj->IsSelected() )
					obj->SetRect( obj->GetLeft(), obj->GetTop(), obj->GetRight(), obj->GetTop() + height );
...
void CDiagramEditor::MakeSameHorizontalPlane()
/* ============================================================
	Function :		CDiagramEditor::MakeSameHorizontalPlane
	Description :	Makes all selected objects the same size as 
					the top selected object in the data container.
	Access :		Public
   ...
   ============================================================*/
{
	...
	if( GetSelectCount() > 1 )
	{
		...
		if( obj )
		{
			double fpHorizontalPlane = obj->GetTop() + ((obj->GetBottom() - obj->GetTop()) / 2);
			int count = 0;
			while( ( obj = m_objs->GetAt( count++ ) ) )
				if( obj->IsSelected() )   {
                    double fpHalfDimension = (obj->GetBottom() - obj->GetTop())/2;
					obj->SetRect( obj->GetLeft(),  fpHorizontalPlane-fpHalfDimension, 
                                  obj->GetRight(), fpHorizontalPlane+fpHalfDimension );
...
void CDiagramEditor::MakeSameVerticalPlane()
/* ============================================================
	Function :		CDiagramEditor::MakeSameVerticalPlane
	Description :	Makes all selected objects the same size as 
					the top selected object in the data container.
	Access :		Public
   ...
   ============================================================*/
{
	...
	if( GetSelectCount() > 1 )
	{
		...
		if( obj )
		{
			double fpVerticalPlane = obj->GetLeft() + ((obj->GetRight() - obj->GetLeft()) / 2);
			int count = 0;
			while( ( obj = m_objs->GetAt( count++ ) ) )
				if( obj->IsSelected() )   {
                    double fpHalfDimension = (obj->GetRight() - obj->GetLeft())/2;
					obj->SetRect( fpVerticalPlane-fpHalfDimension, obj->GetTop (), 
                                  fpVerticalPlane+fpHalfDimension, obj->GetBottom () );
...
void CDiagramEditor::CenterInDiagramVertical()
/* ============================================================
	Function :		CDiagramEditor::CenterInDiagramVertical
	Description :	Makes all selected objects the same size as 
					the top selected object in the data container.
	Access :		Public
...
{
    CDiagramEntity* obj;
    double fpMin=DBL_MAX, fpMax=0, fpEquiDistance, fpDelta;
    int count = 0;
	...
	if( GetSelectCount() > 1 )
	{
		...
		while( ( obj = m_objs->GetAt( count++ ) ) )
			if( obj->IsSelected() )   {
                if (obj->GetTop ()<fpMin)
                    fpMin=obj->GetTop ();
                if (obj->GetBottom ()>fpMax)
                    fpMax=obj->GetBottom ();
		    }
        CSize size=GetVirtualSize ();
        fpEquiDistance=(size.cy-(fpMax-fpMin))/2;
        fpDelta=fpMin-fpEquiDistance;
        count=0;
		while( ( obj = m_objs->GetAt( count++ ) ) )
			if( obj->IsSelected() )   {
                CRect rect=obj->GetRect ();
                rect.OffsetRect (CPoint (0, (int)-fpDelta));
                obj->SetRect (rect);
            }
		SetModified( TRUE );
		RedrawWindow();
...
void CDiagramEditor::CenterInDiagramHorizontal()
/* ============================================================
	Function :		CDiagramEditor::CenterInDiagramHorizontal
	Description :	Makes all selected objects the same size as 
					the top selected object in the data container.
	Access :		Public
   ...
   ============================================================*/
{
	...
	if( GetSelectCount() > 1 )
	{
        ...
        CSize size=GetVirtualSize ();
        fpEquiDistance=(size.cx-(fpMax-fpMin))/2;
        fpDelta=fpMin-fpEquiDistance;
        count=0;
		while( ( obj = m_objs->GetAt( count++ ) ) )
			if( obj->IsSelected() )   {
                CRect rect=obj->GetRect ();
                rect.OffsetRect (CPoint ((int)-fpDelta, 0));
                obj->SetRect (rect);
            }
		SetModified( TRUE );
		RedrawWindow();
...
void CDiagramEditor::DisposeButtonBottom()
/* ============================================================
	Function :		CDiagramEditor::DisposeButtonBottom
	Description :	Makes all selected objects the same size as 
					the top selected object in the data container.
	Access :		Public
   ...
   ============================================================*/
{
    CDiagramEntity* pButton, *obj;
    CRect rect;
    CSize size;
    double fpMax;
    int count=0, nOffset=m_rightMargin, nWidth, nHeight;
	...
	if( GetSelectCount() >= 1 )
	{
        ...
        while (true)   {
            ...
            if (pButton)   {
                ...
                nWidth=rect.Width ();
                nHeight=rect.Height ();
                rect.top=size.cy-m_bottomMargin-nHeight; rect.left = size.cx-nOffset-nWidth;
                rect.bottom = rect.top+nHeight; rect.right = rect.left + nWidth;
                pButton->SetRect (rect);
                nOffset+=(int)pButton->GetRight ()-(int)pButton->GetLeft () + ARRANGE_BUTTON_OFFSET;
		        SetModified( TRUE );
            }
            else
                break;
...
void CDiagramEditor::DisposeButtonRight()
/* ============================================================
	Function :		CDiagramEditor::DisposeButtonRight
	Description :	Makes all selected objects the same size as 
					the top selected object in the data container.
	Access :		Public
   ...
   ============================================================*/
{
    ...
    CSize size;
    double fpMin;
    int count=0, nOffset=m_topMargin, nWidth, nHeight;
	...
	if( GetSelectCount() >= 1 )
	{
        ...
        while (true)   {
            ...
            if (pButton)   {
                ...
                nWidth=rect.Width ();
                nHeight=rect.Height ();
                rect.top=nOffset; rect.left = size.cx-m_rightMargin-nWidth;
                rect.bottom = rect.top+nHeight; rect.right = rect.left + nWidth;
                pButton->SetRect (rect);
                nOffset+=(int)pButton->GetBottom ()-(int)pButton->GetTop () + ARRANGE_BUTTON_OFFSET;
		        SetModified( TRUE );
            }
            else
                break;
...
bool CDiagramEditor::IsTabVisible () const
{
    return m_nTabMode!=0;
}

void CDiagramEditor::ShowTab (BOOL TabMode)
{
	CDiagramEntity* obj = NULL;
    int count = 0;
...
void CDiagramEditor::GestioneTab (CPoint point)
{
    CDiagramEntity* obj;
    BOOL goon=TRUE;
	...
	while( goon && ( obj = m_objs->GetAt( --count ) ) )
	{
		...
		if( hitCode != DEHT_NONE )   {
		    goon = FALSE;
            if (obj->IsTabbeble ())   {
                int nPrevTabIndex, nTabIndex=m_nTabIndex+1;
                bool b;
                if (b=obj->SelectTab (m_nTabIndex+1, &nPrevTabIndex))   {
                    m_nTabIndex++;
                    if (m_nTabIndex!=nPrevTabIndex)
                        TabModified ();
                }
                TabOrder (obj, nTabIndex, nPrevTabIndex, b);
                RedrawWindow ();
            }    
            else
                ShowTab (FALSE);
...
void CDiagramEditor::TabOrder (CDiagramEntity* obj, int nTabIndex, int nPrevTabIndex, bool b)
{   
    CDiagramEntity* objArray;
    BOOL goon=TRUE;

    int count = GetObjectCount();
	while( goon && ( objArray = m_objs->GetAt( --count ) ) )	  {
        if (objArray!=obj)   {
            objArray->AdjustTabOrder (nTabIndex, nPrevTabIndex, b);
        }
    }
}

void CDiagramEditor::CorrectTabIndex ()
{
	CDiagramEntity* obj = NULL;
    int count = 0, n, i=0, nOffset=0;
    bool bFound;

    n = GetObjectCount();
    while (i<n)   {
        bFound=false; count=0;
	    while ((obj=m_objs->GetAt (count++)))   {
            if (obj->IsTabbeble ())   {
                if (obj->GetTabIndex ()==i+1)   {
                    if (!obj->IsAssigned ())   {
                        obj->Assign (true); bFound=true; break;    
                    }
                }
            }
        }
        if (!bFound)
            ...
            obj->SetTabIndex (i-nOffset+1);
        i++;
    }
    count=0;
	while (obj=m_objs->GetAt (count++))   {
        if (!obj->IsAssigned ()&&obj->IsTabbeble ())   {
            obj->SetTabIndex (i-nOffset+1);
            i++;
        }
        obj->Assign (false);
...
void CDiagramEditor::TabModified ()
{
}

void CDiagramEditor::OnParentNotify(UINT message, LPARAM lParam)
{
