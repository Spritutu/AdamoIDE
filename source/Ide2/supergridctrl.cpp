...
CSuperGridCtrl::CSuperGridCtrl()
{
	...
	m_brushErase.CreateSolidBrush(RGB(255,2255,255));
	m_himl = NULL;
    memset (achGraphicsColumns, 0, sizeof (achGraphicsColumns));
}
...
void CSuperGridCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if(lpDrawItemStruct->itemAction == ODA_DRAWENTIRE)
	{
		...
		for(int nColumn=1; GetColumn(nColumn, &lvc); nColumn++)
		{
			...
			{
                if (achGraphicsColumns[nColumn])   {
                    OnPaintColumn (lpDrawItemStruct, nColumn, &rcItem);
                }
                else   {
...
void CSuperGridCtrl::DrawTreeItem(CDC* pDC, CTreeItem* pSelItem, int nListItem, const CRect& rcBounds)
{
	...
	if (bChild)
	{
        ...
        int n;
        if (n=OnDrawPlusMinus (pDC, pSelItem, nListItem, rcBounds))   {
		    CRectangle rect(this, pDC, iIndent, rcBounds);
...
void CSuperGridCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(OnItemLButtonDown(ht))
	{
		...
		if(bSelect && ht.iItem!=-1)
		{
            ...
            if (!achGraphicsColumns[ht.iSubItem])
			    MakeColumnVisible(Header_OrderToIndex(pHeader->m_hWnd, m_CurSubItem));
...
void CSuperGridCtrl::Sort(CTreeItem* pParent, BOOL bSortChildren, int (__cdecl *pSortFun) (const void *, const void *))
{
	...
	if (nChildren > 1)
	{
        ...
        if (!pSortFun)
            pSortFun=CompareChildren;
		qsort(ppSortArray, nChildren, sizeof(CTreeItem*), pSortFun);
...
void CSuperGridCtrl::OnPaintColumn (LPDRAWITEMSTRUCT lpDrawItemStruct, int nColumn, CRect* rc)
{
	return; 
}

int CSuperGridCtrl::OnDrawPlusMinus (CDC* pDC, CTreeItem* pSelItem, int nListItem, const CRect& rcBounds)
{
    return false;
}

void CSuperGridCtrl::SetGraphicalColumn (int nEl, int nIsGraphic)
{
    if (nEl<32)
	    achGraphicsColumns[nEl]=nIsGraphic; 
}
