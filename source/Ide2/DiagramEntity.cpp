...
BOOL CDiagramEntity::IsTabbed() const
{
    return m_nTabMode;
}
   ...
   ============================================================*/
{
	...
	if( menu.CreatePopupMenu() )
	{
		...
		menu.AppendMenu( MF_STRING, CMD_CUT, LOADSTRING (IDS_ADAMODIAGRAMENTITY_1) );
		menu.AppendMenu( MF_STRING, CMD_COPY, LOADSTRING (IDS_ADAMODIAGRAMENTITY_2) );
		menu.AppendMenu( MF_STRING, CMD_DUPLICATE, LOADSTRING (IDS_ADAMODIAGRAMENTITY_3) );
		menu.AppendMenu( MF_SEPARATOR );
		menu.AppendMenu( MF_STRING, CMD_UP, LOADSTRING (IDS_ADAMODIAGRAMENTITY_4) );
		menu.AppendMenu( MF_STRING, CMD_DOWN, LOADSTRING (IDS_ADAMODIAGRAMENTITY_5) );
		menu.AppendMenu( MF_STRING, CMD_FRONT, LOADSTRING (IDS_ADAMODIAGRAMENTITY_6) );
		menu.AppendMenu( MF_STRING, CMD_BOTTOM, LOADSTRING (IDS_ADAMODIAGRAMENTITY_7) );
		menu.AppendMenu( MF_SEPARATOR );
		menu.AppendMenu( MF_STRING, CMD_PROPERTIES, LOADSTRING (IDS_ADAMODIAGRAMENTITY_8) );
...
void CDiagramEntity::DrawTabNumber ( CDC* dc, double zoom )
{
    if (IsTabbed () )   {
	    SetZoom( zoom );
	    CRect rect( round( GetLeft() * zoom ), 
...
void CDiagramEntity::SetTabMode (BOOL TabMode)
{
    if (TabMode)   {
        LoadTabIndex ();
        m_bAssigned=false;
...
int CDiagramEntity::LoadTabIndex ()
{
    return 0;
}

void CDiagramEntity::SaveTabIndex ()
{
}

bool CDiagramEntity::IsTabbeble ()
{
    return true;
}

bool CDiagramEntity::SelectTab (int nTabIndex, int* nPrevTabIndex)
{
    bool b=false;

    *nPrevTabIndex=m_nTabIndex;
    if (!m_bAssigned)   {
        b=m_bAssigned=true;
        m_nTabIndex=nTabIndex;
    }
    else
        m_nTabIndex=nTabIndex-1;    
    return b;
}

void CDiagramEntity::DrawTab( CDC* dc, CRect rect ) const
{
    CString str;
    CBrush brBack;
    CPen   pen;
    CRect  rc ((int)m_left-10, (int)m_top-10, (int)m_left+10, (int)m_top+10);
    
    if (m_nTabIndex>0)   {
        str.Format ("%d", m_nTabIndex);
        brBack.CreateSolidBrush (RGB (128, 128, 64));
        CBrush* pbr=(CBrush*)dc->SelectObject (brBack);
        COLORREF cr=dc->SetTextColor (RGB (255, 255, 255));
...
bool CDiagramEntity::AdjustTabOrder (int nTabIndex, int nPrevTabIndex, bool b)
{
    bool bBack=false;

    if (!b)   {
        if (m_nTabIndex==nTabIndex-1)   {
            m_nTabIndex=nPrevTabIndex; bBack=true;
        }
    }
    else   {
        if (m_nTabIndex>=nTabIndex&&m_nTabIndex<nPrevTabIndex)   {
            m_nTabIndex++; bBack=true;
        }
    }
    return bBack;
}
