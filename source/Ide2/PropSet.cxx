...
void PropSet::Set(const char *key, const char *val, int lenKey, int lenVal) {
	...
	for (PropertySci *p = props[hash % hashRoots]; p; p = p->next) {
		if ((hash == p->hash) &&
	...
	// Not found
	PropertySci *pNew = new PropertySci;
	if (pNew) {
		pNew->hash = hash;
...
SString PropSet::GetWild(const char *keybase, const char *filename) {
	for (int root = 0; root < hashRoots; root++) {
		for (PropertySci *p = props[root]; p; p = p->next) {
	...
	if (superPS) {
		// Failed here, so try in base PropertySci set
...
void PropSet::Clear() {
	for (int root = 0; root < hashRoots; root++) {
		PropertySci *p = props[root];
		while (p) {
			PropertySci *pNext = p->next;
			p->hash = 0;
			delete []p->key;
			p->key = 0;
			delete []p->val;
...
char *PropSet::ToString() {
	...
	for (int r = 0; r < hashRoots; r++) {
		for (PropertySci *p = props[r]; p; p = p->next) {
			len += strlen(p->key) + 1;
			len += strlen(p->val) + 1;
		}
	}
...
bool PropSet::GetFirst(char **key, char **val) {
	for (int i = 0; i < hashRoots; i++) {
		for (PropertySci *p = props[i]; p; p = p->next) {
			if (p) {
				*key = p->key;
				*val = p->val;
...
bool PropSet::GetNext(char ** key, char ** val) {
	...
	for (int i = enumhash; i < hashRoots; i++) {
		if (!firstloop)
			enumnext = props[i]; // Begin with first PropertySci in block
		// else : begin where we left
		firstloop = false;

		for (PropertySci *p = enumnext; p; p = p->next) {
			if (p) {
				*key = p->key;
				*val = p->val;
